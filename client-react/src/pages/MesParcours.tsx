  ...
  return (
    <PageLayout>
      ...
      <ul className="space-y-3">
        {liste.map((p) => (
          <li key={p.id} className="carte p-4 flex items-center gap-4 hover:shadow-card-lg transition-shadow">
            <Link to={`/parcours/${p.id}`} className="flex-1 min-w-0 cursor-pointer">
              <p className="font-heading font-semibold text-encre truncate">{p.intention}</p>
              <p className="text-xs text-brume mt-1">
                {LIBELLES_VISIBILITE[p.visibilite] ?? p.visibilite} · mis à jour le{' '}
                {new Date(p.misAJourLe).toLocaleDateString('fr-FR')}
              </p>
            </Link>
            <button
